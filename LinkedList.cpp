...
template<typename ItemType>
void LinkedList<ItemType>::swap(Node<ItemType> *x, Node<ItemType> *y) {
    Node<ItemType>* prev = nullptr;
    Node<ItemType>* iterator = getHeadPtr();
    while (iterator != nullptr){
        if (iterator->getNext() == x){
            prev = iterator;
        }
        iterator = iterator->getNext();
    }
    Node<ItemType>* temp = y->getNext();
    y->setNext(prev->getNext());
    prev->setNext(y);
    x->setNext(temp);
    temp = nullptr;
...
template<typename ItemType>
void LinkedList<ItemType>::bubbleSortLessToGreat() {
    int size = getSize();
    bool swapped = true;
    int pass = 1;
    Node<ItemType>* iterator = getHeadPtr();
